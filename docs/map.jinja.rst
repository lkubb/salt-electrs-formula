...
We will see a quick setup to configure the ``electrs`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``electrs`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``electrs`` formulas under ``/srv/salt/electrs/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/electrs/parameters/dns:domain/
    ln -s dns:domain /srv/salt/electrs/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/electrs/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/electrs/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``electrs`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``electrs`` formulas under ``/srv/salt/electrs/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/electrs/parameters/roles
...
We create a configuration for the role ``electrs/server`` in ``/srv/salt/electrs/parameters/roles/electrs/server.yaml``:
...
We create another configuration for the role ``electrs/client`` in ``/srv/salt/electrs/parameters/roles/electrs/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles electrs/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles electrs/client
...
.. code-block:: console
    ...
    mv /srv/salt/electrs/parameters/dns:domain/  '/srv/salt/electrs/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-electrs-as-json:
      test.nop:
        - name: {{ electrs | json }}
...
Here is an example based on `template-formula/electrs/config/file.sls`_:
    ...
    electrs-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='electrs-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ electrs.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``electrs`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ electrs['winner'] }}
...
.. _template-formula/electrs/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/electrs/config/file.sls
